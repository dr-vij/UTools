    ...
    private void AfterGUI(SceneView sceneView)
    {
        ...
        switch (e.type)
        {
            ...
            case EventType.MouseUp:
                ...
                if (e.button == 0 && e.alt)
                {
                    if (EditorApplication.timeSinceStartup - m_MouseDownTimeWithAlt <= m_MouseDownClickTimeWithAlt && (m_MouseDownTimeWithAltPosition - e.mousePosition).magnitude < m_MouseClickDistance)
                    {
                        SetCameraPlaneToPosition2d(sceneView, e.mousePosition);
                    }
